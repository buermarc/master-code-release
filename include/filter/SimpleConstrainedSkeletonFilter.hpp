...
class SimpleRigidJointConstructFilter3 {
...
public:
    ...
    static SimpleRigidJointConstructFilter3<Value> default_init(std::vector<int> joints, MatrixXd measurement_errors)
    {
        ...
        */
        ...
        return SimpleRigidJointConstructFilter3<Value>(
            Al,
            Cl,
            Gl,
            phi_1,
    ...
    SimpleRigidJointConstructFilter3(
        MatrixXd ad,
        MatrixXd c,
        MatrixXd gd,
        MatrixXd m_phi_1,
...
template <typename Value>
class SimpleConstrainedSkeletonFilter : public AbstractSkeletonFilter<Value> {
    size_t n_joints;
    ...
    std::unordered_map<int, SimpleRigidJointConstructFilter3<Value>> joint_group_filters;
    std::unordered_map<int, SimplePointFilter3D<Value>> single_joint_filters;

public:
    int joint_count() { return n_joints; }
    bool is_initialized() override { return initialized; }

    SimpleConstrainedSkeletonFilter(
        int m_n_joints,
        double factor,
    ...
    {
        this->set_filter_type("SimpleConstrainedSkeletonFilter");
        this->set_measurement_error_factor(factor);

        for (auto joint_group : m_constrained_joint_groups) {
            auto filter = SimpleRigidJointConstructFilter3<Value>::default_init(joint_group, measurement_errors);
        ...
        for (int i = 0; i < m_n_joints; ++i) {
            if (flat.find(i) == flat.end()) {
                auto filter = SimplePointFilter3D<Value>::default_init(i, measurement_errors);
        ...
        Value new_time) override
    {
        ...
        for (auto& [i, filter] : single_joint_filters) {
            // std::cout << "Filter " << i << std::endl;
            auto [position, velocity, predicted] = filter.step_(values[i], time_diff);
            positions[i] = position;
            velocities[i] = velocity;
...
template <typename Value>
class SimpleConstrainedSkeletonFilterBuilder : public AbstractSkeletonFilterBuilder<Value> {
    int m_joint_count;
    double m_factor;

public:
    SimpleConstrainedSkeletonFilterBuilder(int joint_count, double measurement_error_factor)
    ...
    {
        return std::make_shared<SimpleConstrainedSkeletonFilter<Value>>(m_joint_count, m_factor, get_cached_measurement_error(m_factor), get_azure_kinect_com_matrix());
    }
};
