...
#include "SimpleGenericFilter1D.hpp"
#include "Point.hpp"

using Eigen::MatrixXd;

std::tuple<MatrixXd, MatrixXd, MatrixXd> let_simple_matricies_appear_magically();

template <typename Value>
class SimplePointFilter3D {
    SimpleGenericFilter1D<Value> x_filter;
    SimpleGenericFilter1D<Value> y_filter;
    SimpleGenericFilter1D<Value> z_filter;

public:
    static SimplePointFilter3D<Value> default_init(int joint, MatrixXd measurement_errors)
    {
        ...
        return SimplePointFilter3D<Value>(
            measurement_noise,
            system_noise,
            threshold);
    }

    SimplePointFilter3D(Point<Value> measurement_noise, Point<Value> system_noise,
        Value threshold)
    {
        auto [ad, c, g] = let_simple_matricies_appear_magically();
        x_filter = SimpleGenericFilter1D<Value>(ad,
            ad, // FIXME: we do not care
            c, g,
            g, // FIXME: we do not care
            measurement_noise.x, system_noise.x, threshold);
        y_filter = SimpleGenericFilter1D<Value>(ad,
            ad, // FIXME: we do not care
            c, g,
            g, // FIXME: we do not care
            measurement_noise.y, system_noise.y, threshold);
        z_filter = SimpleGenericFilter1D<Value>(ad,
            ad, // FIXME: we do not care
            c, g,
            g, // FIXME: we do not care
    ...
    {
        auto initial_errors = MatrixXd::Identity(2, 2);
