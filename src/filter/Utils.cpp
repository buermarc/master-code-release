...
legacy_load_filtered_data(std::string path, int joint_counts, int max_frames)
{
    ...
    */
    ...
    Tensor<double, 3, Eigen::RowMajor> joints(n_frames + 1, joint_counts, 3);
    std::vector<double> timestamps;
...
MatrixXd get_cached_measurement_error(double factor)
{
    ...
    return factor * var.array().sqrt();
}

MatrixXd _get_measurement_error(Tensor<double, 3, Eigen::RowMajor> joints, int joint_counts,
    int frame_start, int frame_end)
{
    MatrixXd var(joint_counts, 3);
    Eigen::array<Eigen::Index, 3> offsets;
    Eigen::array<Eigen::Index, 3> extents;
    Tensor<double, 0, Eigen::RowMajor> mean_t;
    Tensor<double, 0, Eigen::RowMajor> sum_t;
